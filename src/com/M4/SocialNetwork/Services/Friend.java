...
public class Friend {
	...
	public String confirmFriend(
		...
		System.out.println(receptionEmail + " " + receptionPassword + " "
				+ senderEmail);
		UserController controller = new UserController();
		JSONObject json = new JSONObject();
		try {
			if (controller.getUser(receptionEmail, receptionPassword) != null
					&& controller.requestExistBefore(senderEmail,
							receptionEmail)
...
//	}
	...
	@POST
	@Path("/GetUserFriends")
	public String getUserFriends(@FormParam("userEmail") String userEmail) {
		ArrayList<User> usersFriends = new UserController()
				.getAllUserFriends(userEmail);
		JSONObject jsonObject = new JSONObject();
		Gson gson = new Gson();
		try {
			jsonObject.put("Status", "OK");
			jsonObject.put("friend", gson.toJson(usersFriends));
